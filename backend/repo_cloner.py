...
def _measure_repo_size(path: Path) -> int:
    """Return total size in bytes of all files under path."""
    return sum(f.stat().st_size for f in path.rglob("*") if f.is_file())


def validate_git_url(repo_url: str) -> str:
    """Normalize and validate a public Git HTTPS URL (GitHub, GitLab, Bitbucket)."""
    parsed = urlparse(repo_url.strip())
    host = parsed.netloc.lower()
    if parsed.scheme != "https" or host not in _ALLOWED_HOSTS:
        raise InvalidRepoUrlError(
            f"Only HTTPS URLs for {', '.join(sorted(_ALLOWED_HOSTS))} are supported."
        )
...
def clone_repository(repo_url: str) -> Path:
    ...
    try:
        Repo.clone_from(
            normalized_url,
            destination,
            depth=50,  # depth 50 covers recent history for Gitleaks secret scanning
        )
    except GitCommandError as exc:
        if destination.exists():
            shutil.rmtree(destination, ignore_errors=True)
